import { useCommentAnnotationsCount, VeltCommentToolWireframe, VeltData, VeltIf } from '@veltdev/react';
  ...
  return (
    <VeltCommentToolWireframe>
      <div className="w-full h-full flex justify-end items-end">
        <button 
          className="relative flex items-center justify-center rounded-full text-black bg-white shadow-md transition-colors w-8 h-8 px-2"
        ...
        >
          ...
          <VeltIf condition="{unreadCommentAnnotationCount} > 0"> 
            <span className="absolute -top-1 -right-1 min-w-[18px] h-[18px] px-1 rounded-full bg-rose-600 text-white text-[10px] leading-[18px] text-center">
              <VeltData field="unreadCommentAnnotationCount"></VeltData>
            </span>
          </VeltIf>
        </button>
      </div>
